...
public class AIOrderDAO { //AIOrderList
        ...
        String placeOfReg, Date revocEffect)
    {
        ...
        /*if (logger.isDebugEnabled())
            logger.debug("AI # " + groupID +" = "+orderID);*/
        ...
        String nameLO)
    {
        ...
        /*if (logger.isDebugEnabled())
                logger.debug("AI unload # " + groupID +" = "+ai.getOrderID());*/
    ...
    {
        if (null == groupID || null == groupIDs || groupIDs.isEmpty())
            return null;
        /*if ((null == unloadList) || (unloadList.isEmpty()))
            have = false;*/
    ...
    public String getAIName(
        ...
        if (!aiNames.containsKey(gKey)){//The "Double-Checked Locking idiom is Broken"
            ...
            if (null == ai){
                aiNames.put(gKey, "");
                return null;
                //throw new IllegalArgumentException("no ai found, will get nullpointer if continue");
            }
            ...
            if (CCRA == uGroupAIID){
                if (DOCUMENT_LANGUAGE.LOCAL.equals(documentLanguage))
    ...
    {//????????????????wtf??????????????????
        ...
        if (null == (ai = (AIOrder) aiHash.get(uGroupID.toString())))
        {
            return false;
        }
    ...
    private void refreshAIName()//????????????????????????????
    {
        if (ONE_HOUR < (System.currentTimeMillis() - lastRefresh))
        {
            //logger.debug("Refreshing AI's name container.");
    ...
    {
        Iterator itGrouopAIID = aiNames.keySet().iterator();
