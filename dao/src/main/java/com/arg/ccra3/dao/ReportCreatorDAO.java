...
import com.arg.ccra3.model.api.DataBlockStream;
import com.arg.ccra3.model.api.MalProductDeliverDataAPI;
import com.arg.ccra3.model.report.Attention;
...
public class ReportCreatorDAO{
    ...
    private static final org.apache.log4j.Logger logger = LogManager.getLogger(ReportCreatorDAO.class);
            ...
            (ResultSet rst) -> {
                ...
                do{
                    header.put("reportName", rst.getInt("objectid"))
                        ...
                        .put("execcompanyName", rst.getString("companyname"));
...
/*    private DataSPM[] findObjectDataDetails(int objectID)
        throws Exception
    {
        final List<DataSPM> result = new LinkedList<>();
        jdbcTemplate.query(
            ReportResource.getString("reportcreator.preparedstatement.datas"),
            (ResultSet rs) -> {
                do{
                    int temp_int = rs.getInt("dataid");

                    DataSPM data = new DataSPM();
                    data.setDataID(temp_int);
                    temp_int = rs.getInt("parentdataid");
                    data.setParentDataID(temp_int);
                    data.setDescription(rs.getString("datadesc"));
                    data.setLabel(rs.getString("datalabel"));
        ...
        return result.toArray(DataSPM[]::new);
    }*/
