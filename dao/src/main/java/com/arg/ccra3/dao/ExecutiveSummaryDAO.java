...
public class ExecutiveSummaryDAO {
    private static final org.apache.log4j.Logger logger = LogManager.getLogger(ExecutiveSummaryDAO.class);
    ...
    private String getFirstReportDate(int cbuid)
        
    {
        Map<String, Object> rst = jdbcTemplate.queryForMap(
            getString("reportcreator.preparedstatement.executivesummary.firstreporteddate"),
            cbuid
        );
        
        return rst != null ? (String) rst.get(getString("report.table.prod_demographic_flag.columns.firstreporteddate")) : null;
    }
    
    private int getTotalAIsReportZeroFacAfterOptout(int cbuid)
    {
        return jdbcTemplate.queryForObject(
            getString("reportcreator.preparedstatement.executivesummary.totalreportdatawithzerooptout"),
            Integer.class,
            new Object[]{ cbuid, getPeriod(2) }
        );
    }

    private TotalAIsReportData getTotalAIsReport(int cbuid) 
    {
        Map<String, Object> rst = jdbcTemplate.queryForMap(
            getString("reportcreator.preparedstatement.executivesummary.totalreportdata"),
            new Object[]{ cbuid, getPeriod(2) }
        );
        
        TotalAIsReportData ret = null;
        if(rst != null)
            ret = new TotalAIsReportData(
                (String)rst.get(getString("report.dynamic.columns.period")),
                (int)rst.get(getString(DYNAMIC_COLUMNS_TOTAL))
            );
        return ret;
    }
