/*com.arg.cb2.inquiry.ExportFile from Batch Inquiry Archiver for CCRA II

<parent>
    <groupId>arg.ccra2</groupId>
    <artifactId>ccra2-parent</artifactId>
...
<name>Batch Inquiry Archiver for CCRA II</name>
*/
...
import com.arg.ccra3.model.report.ReportFinancial;
import com.arg.ccra3.model.report.ReportGeneral;
...
import com.arg.ccra3.model.report.PositiveLoan;
...
public class ReportJsonConvertor {
    ...
    public ReportGeneral toGeneral(Map<String,Object> reports) throws Exception{
        //https://stackoverflow.com/questions/54683821/LinkedHashMap<String, Object>-always-returns-empty-false
        try{
            ...
            throws Exception
    {
        ...
        if (fDate.isBlank() && (
            ...
            execCompanyName = (String) rhd.get("execcompanyName");
         }
        execSum.put("execCompanyName", execCompanyName);
        
        return execSum;
    }
    
    private List<Object> getProfilesJson(Map<String,Object> reports)throws Exception
    {
    ...
    private List<Object> getComStructuresJson(Map<String,Object> reports)throws Exception
    {
            ...
            throws Exception
    {
        ...
        for (SCourt cs : csData)
        {
            ...
            /*List<Object> otherDefs = new ArrayList<>();
            List<Object> otherDefsLO = new ArrayList<>();
            boolean first = true;
            for (SDefendant defIt : def)
            {
                if(first){
                    first = false;
                    continue;
                }
                SDefendantLO d_lo = (SDefendantLO) defIt;
                if (null == d_lo) continue;
                
                otherDefs.put(getDefendantName(d_lo.getGivennameEN(), d_lo.getSurnameEN()));
                String defLoName = toDataInCn(getDefendantName(d_lo.getGivennameLO(), d_lo.getSurnameLO()));
                if(defLoName != null)
                    otherDefsLO.put(defLoName);
            }
            if(!otherDefsLO.isEmpty())
                court.put("otherDefendantLO", otherDefsLO);
            if(!otherDefs.isEmpty())
                court.put("otherDefendant", otherDefs);*/
    ...
    /*private void getCourtDefendant(SCourtLO cs_lo)
    {
        ...
        if (reportTypeID == InquiryConstants.OBJECT_TYPE.BASIC_REPORT)
        {
            getDataInChineseDefendant(d_lo, ExportFileElements.DEFENDANT_LO);
        }
        else if (reportTypeID == InquiryConstants.OBJECT_TYPE.CHINESE_REPORT)
        {
        ...
        for (int i = 1; i < d.length; i++)
        {
            ...
            if (reportTypeID == InquiryConstants.OBJECT_TYPE.BASIC_REPORT)
            {
                getDataInChineseDefendant(d_lo,
                    ...
                    == InquiryConstants.OBJECT_TYPE.CHINESE_REPORT)
            {
                                           ...
                                           String element)
    {
        if (null != d_lo)
        {
            ...
            {
                XMLUtil.getCData(
                        ReportResourceJson.getMessage(userPrefLang.getLocale(),
    ...
    private List<Object> getNoticeOfConsentJson(Map<String,Object> reports)throws Exception
    {
    ...
    public ReportFinancial toFinancial(Map<String,Object> reports) throws Exception{
        try{
    ...
    private PositiveLoan getPositiveFinanceJson(SPositiveFinancials psf) throws Exception
    {
    ...
    private LinkedHashMap<String, Object> getPositiveFinancelSummary(SPositiveFinancials psf) throws Exception
    {
    ...
    private List<Object> getSubmitterWithOutFacCancelJson(SPositiveFinancials psf) throws Exception{
    ...
    private List<Object> getSubmitterWithFacCancelJson(SPositiveFinancials psf) throws Exception{
    ...
    private LinkedHashMap<String, Object> getNegativeFinancialJson(SNegativeFinancials nf) throws Exception{
    ...
    private LinkedHashMap<String, Object> getNegativeSummary(SNegativeFinancial nf12, SNegativeFinancial nf24) throws Exception{
    ...
    private LinkedHashMap<String, Object> getNegatovePastDue(SNegativeFinancials nf) throws Exception{
    ...
    private void setNegatovePastDueData(List<Object> data, SNegativeFinancial[] negativeFinancialData) throws Exception
    {
    ...
    private LinkedHashMap<String, Object> getWriteOff(SWriteOffFinancials wof) throws Exception{
