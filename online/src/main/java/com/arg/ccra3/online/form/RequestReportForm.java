...
public class RequestReportForm {
    ...
    /*public void reset(ActionMapping mapping, HttpServletRequest request)
    {
        ...
        this.basic = Boolean.valueOf(request.getParameter("basic"))
                                .booleanValue();
        this.chinese = Boolean.valueOf(request.getParameter("chinese"))
                                  .booleanValue();
        this.monitoring = Boolean.valueOf(request.getParameter("monitoring"))
                                     .booleanValue();
        this.enquiry = Boolean.valueOf(request.getParameter("enquiry"))
                                  .booleanValue();
        this.recordPerPage = request.getParameter("recordPerPage");
        this.basicOrdered = Boolean.valueOf(request.getParameter("basicOrdered"))
                                       .booleanValue();
        this.sevenDayBasic = Boolean.valueOf(request.getParameter(
                    "sevenDayBasic")).booleanValue();
        this.sevenDayChinese = Boolean.valueOf(request.getParameter(
                    "chineseAvailable")).booleanValue();
        this.chineseAvailable = Boolean.valueOf(request.getParameter(
                    "chineseAvailable")).booleanValue();
        ...
        this.reportRefNumb = request.getParameter("reportRefNumb");
        this.newSearch = Boolean.valueOf(request.getParameter("newSearch"))
                                    .booleanValue();
        this.charge = Boolean.valueOf(request.getParameter("charge"))
                                 .booleanValue();
        this.transactionId = request.getParameter("transactionId");
        this.monitorBuyer = request.getParameter("monitorBuyer");
        this.enquiryBuyer = request.getParameter("enquirybuyer");
        this.yourEnquiry = Boolean.valueOf(request.getParameter("yourEnquiry"))
                                      .booleanValue();
        this.yourMonitoring = Boolean.valueOf(request.getParameter(
                    "yourMonitoring")).booleanValue();

        Object expObj = request.getAttribute("expenseID");

        if (null == expObj)
        {
            this.setExpenseID(request.getParameter("expenseID"));
        }
        else
        {
            this.setExpenseID((String) expObj);
            this.expenseID = (String) expObj;
        }

        if ((null == (this.typeOfReport = request.getParameter("typeOfReport")))
                || (typeOfReport.length() == 0))
        {

            this.typeOfReport = String.valueOf(request.getAttribute(
                        "typeOfReport"));
        }

        if (null == (this.transactionId = request.getParameter("transactionId")))
        {

            this.transactionId = String.valueOf(request.getAttribute(
                        "transactionId"));
        }
    }*/
    ...
    {
        if ((null == cbuid) || (0 == cbuid.length()))
        {
            errors.add(ActionMessages.GLOBAL_MESSAGE,
                new ActionMessage("WBS-A00001"));

            log.error("Bug Tracking ID 20070803_WBS_1 :CBUID");
        }

        if ((null == reasonCode) || (0 == reasonCode.length()))
        {
            errors.add(ActionMessages.GLOBAL_MESSAGE,
                new ActionMessage("WBS-A00008"));

            log.error("Bug Tracking ID 20070803_WBS_1 :REAQSONCODE");
        }

        if ((null == companyName) || (0 == companyName.length()))
        {
            errors.add(ActionMessages.GLOBAL_MESSAGE,
                new ActionMessage("WBS-A00009"));

            log.error("Bug Tracking ID 20070803_WBS_1 :COMPANYNAME");
        }

        if (!errors.isEmpty())
        {
            log.error("Bug Tracking ID 20070803_WBS_1 :" +this.toString());
        }
